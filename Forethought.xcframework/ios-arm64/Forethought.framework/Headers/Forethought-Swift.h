...
#ifndef FORETHOUGHT_SWIFT_H
#define FORETHOUGHT_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="Forethought",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
@class ForethoughtTriggerEventData;
...
/// A trigger event step was reached in a workflow. Do your native work, then call
/// <code>ForethoughtSDK.sendTriggerEventResponse(identifier:payload:)</code> to resume the conversation.
- (void)triggerEventReceivedWithTriggerEventData:(ForethoughtTriggerEventData * _Nonnull)triggerEventData;
@end
...
/// Tells the Forethought SDK that the handoff was successfully shown to the user
/// \param success A bool deciding if the handoff was successful or not
///
+ (void)sendHandoffResponseWithSuccess:(BOOL)success;
/// Responds to a trigger event step, resuming the conversation with the resolved context variables
/// <ul>
///   <li>
///     Parameters:
///   </li>
///   <li>
///     identifier: The event name of the trigger event step. Use <code>ForethoughtTriggerEventData.name</code>.
///   </li>
///   <li>
///     payload: The context variables to return, keyed by context variable name
///   </li>
/// </ul>
+ (void)sendTriggerEventResponseWithIdentifier:(NSString * _Nonnull)identifier payload:(NSDictionary<NSString *, id> * _Nonnull)payload;
/// Clears the widget’s locally stored data (e.g. conversation history)
/// note:
/// This is a no-op unless the Forethought Solve view is currently shown
/// (<code>show(...)</code> has been called) and the widget has finished loading. There is no
/// webview to receive the command otherwise.
+ (void)clearLocalData;
/// Launches the widget with an initial query on the user’s behalf
/// note:
/// This is a no-op unless the Forethought Solve view is currently shown
/// (<code>show(...)</code> has been called) and the widget has finished loading. There is no
/// webview to receive the command otherwise.
/// \param query The initial query to submit
///
+ (void)launchQuery:(NSString * _Nonnull)query;
/// Updates the conversation context with the given key/value pairs
/// note:
/// This is a no-op unless the Forethought Solve view is currently shown
/// (<code>show(...)</code> has been called) and the widget has finished loading. There is no
/// webview to receive the command otherwise.
/// \param context The context values to merge into the conversation
///
+ (void)updateConversationContext:(NSDictionary<NSString *, id> * _Nonnull)context;
/// Updates the widget configuration parameters
/// note:
/// This is a no-op unless the Forethought Solve view is currently shown
/// (<code>show(...)</code> has been called) and the widget has finished loading. There is no
/// webview to receive the command otherwise.
/// \param params The config parameters to apply
///
+ (void)updateConfigParams:(NSDictionary<NSString *, id> * _Nonnull)params;
...
/// A structured object for a trigger event step fired from a workflow
SWIFT_CLASS("_TtC11Forethought27ForethoughtTriggerEventData")
@interface ForethoughtTriggerEventData : NSObject
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
